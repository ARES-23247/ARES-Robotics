...
import com.areslib.frc.updateState
...
import org.aresfirst.marvin.robot.FRCAutoOrchestrator
import org.aresfirst.marvin.robot.FrcLocalizationCalibrationControls
import org.aresfirst.marvin.robot.FrcAutoCapabilities
import org.aresfirst.marvin.robot.FRCTeleOpDriveController
import org.aresfirst.marvin.robot.FrcSysIdController
...
import org.aresfirst.marvin.generated.subsystems.GeneratedSubsystemRegistry
import org.aresfirst.marvin.generated.subsystems.superstructure.GeneratedSuperstructureRegistry
import org.aresfirst.marvin.sim.FrcDashboardDriveInput
import org.aresfirst.marvin.sim.applyTo
    ...
    fun load(bytes: ByteArray): FrcFieldContract? {
        ...
        val config = runCatching {
            com.areslib.state.ValidatedRobotFieldLoader.load(
                bytes,
                requiredFieldType = com.areslib.state.FieldType.FRC,
...
class ARESRobot : TimedRobot() {
    ...
    private lateinit var teleopGeneratedCapabilities: org.aresfirst.marvin.generatedruntime.FrcGeneratedRoutineCapabilities
    ...
    private val can2Bus = com.ctre.phoenix6.CANBus(
        org.aresfirst.marvin.generated.drivebase.GeneratedAresDrivebaseConfig.CTRE_CAN_BUS
    )
    ...
    override fun robotInit() {
        ...
        ).mapNotNull { it as? org.aresfirst.marvin.hardware.FrcMechanismConfigurationStatus }
            .toTypedArray()
        ...
        val mechanismHomingDevices = if (isReal) {
            arrayOf(
                cowlIO as org.aresfirst.marvin.hardware.FrcMechanismHomingStatus,
                intakeIO as org.aresfirst.marvin.hardware.FrcMechanismHomingStatus,
                climberIO as org.aresfirst.marvin.hardware.FrcMechanismHomingStatus
            )
        } else {
            emptyArray()
        }
        ...
        val initialState = RobotState(
            ...
            tuning = org.aresfirst.marvin.config.CanonicalDrivebaseConfig.initialTuningState(),
        )
        ...
        mechanismCommissioning = FrcMechanismCommissioningController(
            ...
            flywheelTuningStatus = flywheelIO as? org.aresfirst.marvin.hardware.FrcFlywheelTuningStatus,
        )

        val tuningRuntime = org.aresfirst.marvin.generated.drivebase.GeneratedAresTuningConfig.createRuntime()
        robot.tuningManager = com.areslib.tuning.TuningManager(
            ...
            onApplied = { parameterUid, _ ->
                if (org.aresfirst.marvin.config.CanonicalDrivebaseConfig.supportsRuntimeParameter(parameterUid)) {
                    robot.store.dispatch(
                        RobotAction.UpdateTuningState(
                            org.aresfirst.marvin.config.CanonicalDrivebaseConfig.withRuntimeValues(
            ...
            isConsumerSupported = org.aresfirst.marvin.config.CanonicalDrivebaseConfig::supportsRuntimeParameter,
        )
        ...
        if (swerveIO != null) {
            hardwareRegistry.registerDevice(
                ...
                com.areslib.telemetry.schema.TopologyNode(
                    ...
                    displayName = "CTRE Swerve (${org.aresfirst.marvin.generated.drivebase.GeneratedAresDrivebaseConfig.CTRE_CAN_BUS})",
            ...
            if (action is RobotAction.CalibrateSwerveOffsets && swerveIO != null) {
                ...
                if (swerveCalibrationSamples.copyFresh(nowMs, calibrationEncoderPositions)) {
                    val activeOffsets = com.areslib.drivetrain.SwerveOffsetManager.loadOffsets(
                        org.aresfirst.marvin.config.CanonicalDrivebaseConfig.profiledOffsets()
                    )
        ...
        teleopGeneratedCapabilities = org.aresfirst.marvin.generatedruntime.FrcGeneratedRoutineCapabilities(robot)
