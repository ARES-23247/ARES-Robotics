...
import org.aresfirst.marvin.sim.Dyn4jSimTelemetryPublisher
import org.aresfirst.marvin.sim.Dyn4jSwerveModuleSim
...
class Dyn4jSimulation(
    ...
    private val physicsWorld = Dyn4jPhysicsWorld(
        robotLengthMeters = org.aresfirst.marvin.config.CanonicalDrivebaseConfig.simulationRobotLengthMeters,
        robotWidthMeters = org.aresfirst.marvin.config.CanonicalDrivebaseConfig.simulationRobotWidthMeters,
    )
    private val swerveSim = Dyn4jSwerveModuleSim(
        kpLinear = org.aresfirst.marvin.config.CanonicalDrivebaseConfig.simulationLinearKp,
        kpAngular = org.aresfirst.marvin.config.CanonicalDrivebaseConfig.simulationAngularKp,
    )
    ...
    val flywheelIO: FlywheelIO = org.aresfirst.marvin.sim.io.SimulatedFlywheelIO(this)
    val cowlIO: CowlIO = org.aresfirst.marvin.sim.io.SimulatedCowlIO(this)
    val intakeIO: IntakeIO = org.aresfirst.marvin.sim.io.SimulatedIntakeIO(this)
    val feederIO: FeederIO = org.aresfirst.marvin.sim.io.SimulatedFeederIO(this, feederPieceDetectorConfigured)
    val floorIO: FloorIO = org.aresfirst.marvin.sim.io.SimulatedFloorIO(this)
    val climberIO: ClimberIO = org.aresfirst.marvin.sim.io.SimulatedClimberIO(this)
    ...
    fun step(state: RobotState, dt: Double): List<RobotAction> {
        ...
        if (flywheelAtSpeed && feederSpinning && state.superstructure.marvin.inventoryCount > 0 && shootCooldownTimer <= 0.0) {
            ...
            if (org.aresfirst.marvin.marvin.MarvinConfig.SHOT_CONFIG.shooterFacesRearward) {
