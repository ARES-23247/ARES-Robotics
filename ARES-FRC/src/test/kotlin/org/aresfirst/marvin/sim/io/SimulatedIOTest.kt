...
import org.aresfirst.marvin.marvin.FlywheelState
...
class SimulatedIOTest {
    ...
    fun testAllSimulatedIOs() {
        Dyn4jSimulation(seed = 42L).use { sim ->

            // 1. Cowl
            val cowl = SimulatedCowlIO(sim)
            cowl.setTargetAngle(10.0, 1.0)
            ...
            climber.setTargetPositionRotations(0.1, 1.0)
            ...
            flywheel.setVelocityRpm(4000.0, 1.0)
    ...
    fun feederDetectorIsOnlyValidWhenExplicitlyConfigured() {
        Dyn4jSimulation(seed = 42L).use { unconfiguredSim ->
            ...
            Dyn4jSimulation(seed = 42L, feederPieceDetectorConfigured = true).use { configuredSim ->
    ...
    fun `model matched flywheel control reaches the production readiness band`() {
        Dyn4jSimulation(seed = 42L).use { sim ->
            ...
            repeat(300) {
                flywheel.setVelocityRpm(targetRpm, 1.0)
