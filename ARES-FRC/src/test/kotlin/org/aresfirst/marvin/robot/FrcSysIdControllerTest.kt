...
class FrcSysIdControllerTest {
    ...
    fun `cached overcurrent stops characterization after the stall interval`() {
        ...
        assertEquals(6.0, flywheel.lastAppliedVoltage)
        controller.update(1199L, RobotState(), true, true)
        assertEquals(6.0, flywheel.lastAppliedVoltage)
        controller.update(1200L, RobotState(), true, true)
        assertEquals(0.0, flywheel.lastAppliedVoltage)
        assertEquals("NONE", telemetry.strings["SysId/Status"])
        assertEquals(3, flywheel.currentReads)
    }

    @Test
    fun `invalid or stale cached current stops an active characterization`() {
        for (reading in listOf(Double.NaN, Double.POSITIVE_INFINITY, -1.0, 5.0)) {
            val telemetry = FakeTelemetry().apply { strings["SysId/Command"] = "START_FLYWHEEL_DYNAMIC" }
            val flywheel = FakeFlywheel(1000.0)
            val controller = FrcSysIdController(telemetry, flywheel)
            controller.update(1000L, RobotState(), true, true)
            assertEquals(6.0, flywheel.lastAppliedVoltage)
            flywheel.measuredCurrent = reading
            flywheel.currentFresh = reading != 5.0
            controller.update(1020L, RobotState(), true, true)
            assertEquals(0.0, flywheel.lastAppliedVoltage)
            assertEquals("NONE", telemetry.strings["SysId/Status"])
            assertEquals(2, flywheel.currentReads)
        }
    }
