...
class FRCTeleOpDriveControllerTest {
    ...
    fun `leased dashboard frame reaches FRC teleop and moves dyn4j along field X`() {
        ...
        assertTrue(gate.observe(dashboardFrame(sequence = 0L, flags = flags), timestampMs = 1_000L))
        assertTrue(
            gate.observe(
        ...
        requireNotNull(gate.takeIf { it.receiverReady(1_020L) }).applyTo(controllerState)
        ...
        assertFalse(gate.receiverReady(1_521L), "the receiver lease must expire without new frames")
    }
    ...
    fun `five simulated minutes of FRC dashboard frames retain translation and rotation authority`() {
        ...
        try {
            ...
            repeat(15_000) { index ->
                ...
                val current = requireNotNull(gate.takeIf { it.receiverReady(nowMs) })
            ...
            assertTrue(gate.receiverReady(301_000L), "fresh 50 Hz frames must keep the receiver lease alive")
            assertFalse(gate.receiverReady(301_501L), "the receiver must still fail closed after the soak stops")
        } finally {
            simulation.close()
        }
    }
