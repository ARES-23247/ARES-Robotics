...
class MarvinShooterSubsystemTest {
    ...
    fun `setting target velocity dispatches to IO and respects flywheel speed tolerances`() {
        ...
        // Brownout constrains effort without changing the physical velocity target.
        superstructure.writeOutputs(store.state, scale = 0.5)
        assertEquals(targetRpm, flywheelIO.velocityRpmCommand, 1e-4, "Effort scaling must preserve the target")
        assertEquals(0.5, flywheelIO.effortScale, 1e-4, "The hardware controller must receive the effort limit")
