...
- `Main.kt`'s AWT-thread health check must not call `GetWindowTextLength` / `GetWindowText` while enumerating desktop HWNDs. Those APIs can synchronously message AWT's toolkit window and deadlock the event queue. Filter `EnumWindows` results by current PID and native visibility; title matching belongs in the external capture/interaction scripts.
