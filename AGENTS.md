...
- `Main.kt` must retain an explicit floating, centered `1440 x 900 dp` window, `visible = true`, a `1100 x 700` AWT minimum size, the bounded Compose-owned startup `alwaysOnTop` state, and the `toFront()` / `requestFocus()` presentation calls unless a tested replacement provides the same guarantees.
