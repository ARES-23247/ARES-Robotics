...
/**
 * End-to-end consumer round-trip proof for a generic FTC starter project.
 *
 * Exercises create starter -> save configuration changes across drivetrain, subsystem, tuning, controls,
 * and autonomous -> export -> safe extract -> reopen in fresh session -> independent assertions ->
 * byte-identical USER-OWNED extension -> regenerate with real Gradle wrapper -> multi-file determinism ->
 * verification build compile & tests -> prove simulated IO behavior.
 */
class GenericStarterConsumerRoundtripIntegrationTest {
    @get:Rule
    ...
    fun `generic starter complete consumer roundtrip proves settings, user extensions, determinism, and simulated IO`() = runBlocking {
        ...
        val creationResult = templateService.create(
            RobotProjectCreationRequest(
                ...
                folderName = "ftc-starter-robot",
                league = League.FTC,
                teamId = "23247",
                seasonId = "2026",
                robotId = "RoundtripBot",
                robotName = "Roundtrip Bot",
                templateKind = RobotProjectTemplateKind.GENERIC_STARTER,
            )
        )
        ...
        assertEquals("team23247-roundtripbot", initialDocs.metadata?.projectId)

        // 2. Save meaningful configuration changes across:
        //    (A) Field settings: modify field dimensions to 3.60m x 3.60m
        ...
        val gripperSubsystem = SubsystemTemplates.create(
            ...
            doc.copy(
                hardware = doc.hardware.map { device ->
                    device.copy(connection = device.connection.copy(hardwareMapName = "gripper-${device.hardwareId}"))
                },
        ...
        val modifiedBindings = originalControls.bindings.map { binding ->
            if (binding.bindingId == "drive-forward") {
        ...
        val customizedTeamExtensions = buildString {
            ...
            appendLine("    fun customStarterMetric(): Double = 42.0")
            appendLine("    fun customStarterTag(): String = \"FTC_STARTER_PRESERVED_EXTENSION\"")
            appendLine("}")
        }
        ...
        assertEquals(3.60, extractedDocs.metadata?.fieldLengthMeters ?: 0.0, 1e-6, "Field length must retain saved value")
        assertEquals(3.60, extractedDocs.metadata?.fieldWidthMeters ?: 0.0, 1e-6, "Field width must retain saved value")
        ...
        assertEquals(10, autoEntry.sortOrder, "Autonomous catalog entry sortOrder must retain saved value")
        assertEquals(0.5, autoEntry.startingPose.xMeters, 1e-6, "Autonomous catalog entry starting pose must retain saved value")
        ...
        try {
            ...
            assertTrue(generatedSource.contains("3.6"), "Generated Kotlin must contain reopened field dimension 3.6")
