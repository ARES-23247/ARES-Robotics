...
class SummaryEngineServiceTest {
    ...
    fun testGenerateSummaryEmptySessionReturnsSafeDefaults() = runTest {
        withDatabase { databaseService ->
            ...
            assertEquals(0.0, summary.minBatteryVoltage)
    ...
    private suspend fun withDatabase(block: suspend (DatabaseService) -> Unit) {
        val directory = Files.createTempDirectory("ares-summary-service-test").toFile()
        val database = DatabaseService(directory.resolve("telemetry.duckdb").absolutePath)
        try { block(database) } finally { database.close(); directory.deleteRecursively() }
    }
