...
internal inline fun <T> withDuckDbTransaction(connection: Connection, activateNativeTransaction: Boolean = false, block: () -> T): T {
    ...
    try {
        ...
        if (activateNativeTransaction) connection.createStatement().use { it.execute("SELECT 1") }
        val result = block()
