# ARES Robotics workspace engineering guide

Read the root [shared agent instructions](../../AGENTS.md) first. Paths in code spans
are relative to the monorepo root unless a product section explicitly says otherwise.
Dependency versions below describe the checked-in build; build files, wrapper properties,
and `release/ares-versions.properties` are authoritative when they change.
...
These have caused real bugs. The canonical reference is **`ARESLib-Kotlin/GEMINI.md`, "Coordinate and estimator contract"** — read it before touching coordinates/heading.
...
schema/model/compiler, core, codegen, FTC/FRC hardware and runtime modules, `xrp-hardware`, mocks, simulation
...
- **`core/src/main/kotlin/com/areslib/`** packages:
  ...
  - `networktables/` — custom NT4 server (`NT4Server` :5810, Java-WebSocket + MessagePack)
...
- **Docs:** `ARESLib-Kotlin/GEMINI.md` (contributor rules), `ARESLib-Kotlin/docs/architecture.md`, `ARESLib-Kotlin/docs/onboarding/`, `ARESLib-Kotlin/TEST_INFRA.md`.
...
- `src/main/kotlin/org/aresfirst/marvin/`:
  - `Main.kt` (`object Main`), `ARESRobot.kt` (TimedRobot lifecycle), `Dyn4jSimulation.kt`
  - `hardware/` — `FRCFlywheelHardwareIO` (4× TalonFX dev 9-12), `FRCCowlHardwareIO`, `FRCIntakeHardwareIO`, `FRCFloorHardwareIO`, `FRCClimberHardwareIO`, `FRCFeederHardwareIO`
...
- **`app/src/main/kotlin/com/ares/analytics/`** (MVI: `XxxState` + `sealed XxxIntent` + `XxxViewModel`):
  - `Main.kt` (thin composition root: 1440×900 window + wiring) delegating lifecycle to `desktop/` (`DesktopInstanceLock`, `DesktopCrashHandler`, `DesktopWindowPresentationController`, `NativeWindowProbe`, `DesktopStartupMachine`, `DesktopShutdownCoordinator`), `di/ServiceRegistry.kt` (lazy tiers — **the index into all business logic**)
...
- **`shared/`** — shared JSON (`AppJson`), `Models.kt` (field geometry, Obstacle, GamePiece, AprilTagPlacement), `models/` (Session, SessionSummary, TelemetryFrame, AlertRecord, WorkspaceConfig, TopologyNode, HardwareTopology, ForensicsRequest, DriverProfile).
...
### ARES-XRP-Starter (`ARES-XRP-Starter/`)

The Python-native starter owns `.ares` to MicroPython generation, the bundled `ares_micro`
runtime, `hardware.py` XRPLib boundary, `simulator/`, and `deploy/xrp_device.py`.
Read `ARES-XRP-Starter/README.md` and `docs/PHYSICAL_READINESS.md` within that product.

- Use `ares.bat verify` (`./ares verify` on macOS/Linux), `simulate`, and `plan-deploy` for host validation.
- Generated code and safety tests live under `build/generated/ares`; preserve USER-OWNED `extensions/`.
- Select the exact RP2350 or RP2040 board profile in canonical project identity. Firmware and
  XRPLib pins come from `device/`; preparation checks lengths/hashes and deployment checks
  device identity and APIs. Never bypass mismatches or silently substitute a board profile.
- Deployment verifies and compiles each staged file before switching the active slot. Preserve
  A/B recovery, rollback, and the previous active slot on an interrupted or invalid upload.
- Preserve `ares-xrp/1` on TCP 5811, canonical project fingerprint checks, monotonic sessions
  and sequences, explicit start/arm, deadman neutralization, and latched brownout handling.
- `xrp_secrets.py` is local and ignored. Host tests and device preflight do not constitute
  physical validation; report the actual readiness gates completed.
...
- A fatal uncaught `AWT-EventQueue` exception or unexpected disposal of the only Compose window must run bounded service disposal before terminating. This cleanup owns simulator/build descendants as well as the instance lock; an unusable desktop JVM must not leave either Studio or its managed robot processes alive.
...
- In the `:app:run` task graph, `killExisting` must run after `:app:jar`. A broken replacement build must fail before terminating the current healthy window. Scoped orphan recovery must terminate the verified Studio process-tree descendants before the parent, so a managed simulator cannot be reparented and left alive.
...
- **Opening an older robot project?** Studio requires the project's pinned `aresVersion` to match its own runtime and rejects the retired `ARESLib-Kotlin/maven` channel before starting Gradle, simulation, or deployment. Export a current standalone project or migrate the dependency and USER-OWNED extension APIs together; do not add compatibility shims or silently override the project's declared version.
...
- **Heading/rotation looks wrong?** Re-read `ARESLib-Kotlin/GEMINI.md`, "Coordinate and estimator contract", and the negation rules in §5 above. Usual culprits: extra negation after `PinpointIO`, the `-90°` canvas offset, or Limelight `rotation.y` vs `.z`.
...
- **Tests:** ARESLib and FRC products use JUnit 5. FTC Android and desktop simulator tests
  (including the FTC starter) use JUnit 4. Studio uses Kotlin test with the JUnit 4 runner;
  do not import Jupiter-only APIs into its tests. XRP uses Python unittest and generated safety
  tests. Check the affected module build file before selecting test APIs. Run the dependency-ordered
...
| Dashboard↔topic map | This file §4 (canonical topics) |
...
| PathPlanner file format | `ARESLib-Kotlin/core/src/main/kotlin/com/areslib/pathing/PathPlannerParser.kt` and `PathPlannerJsonParser.kt`; Studio editor state lives in `ARES-Analytics/app/src/main/kotlin/com/ares/analytics/viewmodel/PathPlannerModels.kt` |
